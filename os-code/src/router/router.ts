...
export class Router {
  ...
  /** The chat role used for a delegated subtask, with orchestrator fallback. */
  roleFor(role: 'coding' | 'fast' | 'vision'): { resolved: ResolvedRole; fellBack: boolean } {
  ...
  async delegate(
    ...
    if (fellBack) {
      const note = `No ${role} specialist is enabled, so the orchestrator handled it itself.`;
    ...
    if (role === 'vision') {
      ...
      if (!caps.supportsVision) {
        throw new Error(
          `${resolved.ref.model} cannot read images. Enable a vision specialist (osc market, category "can read screenshots") to analyze screenshots.`,
        );
      }
    }

    const adapter = adapterFor(resolved.ref.model);
    const system = adapter.systemPreamble(
      role === 'vision'
