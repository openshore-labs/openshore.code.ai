...
export const writeFileTool: ToolDef<typeof schema> = {
  ...
  async execute(args, ctx) {
    ...
    // The same structural gate editFile has: a JS or JSON file that does not
    // parse is refused before it lands, and the message says why, so a small
    // model rewriting a whole file cannot leave it broken on disk.
    const structural = structuralCheck(abs, args.content);
    if (!structural.ok) {
      return {
        ok: false,
        content: `The file was NOT written because verification failed: ${structural.detail}\n${before ? 'The existing file is unchanged.' : 'Nothing was created.'} Send the complete, correct content.`,
      };
    }
