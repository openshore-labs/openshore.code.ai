...
Older `## Current state` sections and log entries moved out of `PROGRESS.md`
on 2026-09-05 so that file stays lean (one Current state, one What remains,
...
- **Not verifiable here:** a real scheduled fire under Ollama on the founder's
  ...
  the room on an iPhone (TestFlight). See PROGRESS.md What remains.
...
os-code 744 green (5 new), app typecheck green.
...
**What remains (this arc):** superseded the same day. Both founder asks (the
admin-controlled visibility and the stack-level optimizer) shipped in the entry
above this one (2026-09-04, Stack Health: daily cadence, admin-gated visibility,
Run leaner).
...
Founder, from a phone: Desktop + phone, Walk me through it, then "Why is this necessary?" stopped on the workspace-id error with only Try again, which fails the same way. The guide had opened on a Claude key that needs a workspace id. Now every failed stop's card carries a next move: "Continue with Harbor Lite" (or the Stack off the phone) carries the chat there and resends the message without doubling it in the history (`switchModel(source, { resend })`, `continueElsewhere`); "Choose another model" when nothing else here can answer; "Open Cloud Connections" when the fix lives there; Try again hides for a setup failure. A Claude turn that proves the key's setup wrong sets `cloudKeyNeedsFix`, so the next guide opens on Harbor Lite instead; reconnecting clears it. Pure core `app/src/lib/chatRescue.ts`, tests `app/test/chatRescue.test.ts`. Not yet felt on a phone.
...
### 2026-09-24, the Creative Studio brand sweep, built

Founder: sweep the whole app for brand consistency, address every finding, take the four recommended calls. Plan in `docs/brand-sweep-2026-09-24.md`; built by three parallel builders and merged. Trust: current Harbor facts on the privacy sheet and bylines, every Marketplace door closed, no prices on screen while pay gates are off (`shownPrice`), the model pill names the real provider, a held first message is kept, approvals read plain verbs, deletes use one danger confirm, a BYOM chip says local or cloud by its address (`byomReach`), Forget asks first, sheets close, designed loading/empty/error states (`plainError`). One voice: "Stack", "your computer", room casing from one list (`navRooms.ts`), the glossary, Harbor Lite's cards matched. System: `--accent` for chrome and `--attention` (plum) for "needs you" so teal means local and amber means spend, every raw color tokenized, a type scale and radius tokens, one `Icon` at a 1.8 stroke, the splash as a swell with a dark variant. Moments: the dark splash swell and the landing greeting on the swell once per session. Guards: `color-tokens`, `type-radius-scale`, `copy-glossary`, `confirmGrammar`. Not yet on a device: the iOS dark launch screen (Swift, no Xcode here) and the new icon weight.

### 2026-09-24, the first open writes itself (Creative Studio, "Tide Letter")

Founder: the first open landed on finished text; reveal it, pause on the welcome, then keep the walk moving. The Creative Studio proposed three directions and recommended Tide Letter, now built: 600 ms of stillness, the greeting written two words per 190 ms, a 3 s reading pause from full ink, the walk's opening at three words per 170 ms with a 600 ms breath before the step heading (which lands as one piece), then the buttons settle (primary, then the ask/skip row one stagger behind) with one soft tick. Pure timing in `app/src/lib/introWalk.ts`, driving both the bubble's reveal and the walk's sequencing. A tap on the transcript or the first keystroke finishes it; reduced motion shows it whole; it never replays; VoiceOver reads each line once, whole. Copy tightened by the Studio, and Step 1 no longer implies web search in airplane mode. Tests: `app/test/introWalk.test.ts`. Not yet felt on a phone. Revised the same day (founder: "a wave rolling across the screen"): the Studio's "Swell Line" replaces the word fade. Each scripted line is laid out whole; every character rises a hair, crests (faint brightening in dark), and settles on a delay from one `inkPlan`, many mid-swell at once so it reads as one low wave rolling left to right and onto the next line (`lib/inkSwell.ts`, tokens `--ease-ink`, `--dur-swell`, `--ink-crest`). A skip lands the rest flat within a fifth of a second; a settled line renders plain.
