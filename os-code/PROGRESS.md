...
- [ ] **v1 direction: founder decisions, then Phase 0 (reviewed
      2026-09-25).** `docs/v1-direction-review.md` (memos in
      `docs/v1-direction-advisory-memos.md`; the Zed analysis behind it in
      `docs/zed-proposal.md`). All eight advisors: GO WITH CONDITIONS. As
      written, the definition of done cannot be recorded: verify never runs
      on a phone session (`loop.ts:341`), the floor model is non-commercial,
      and "no one, including us, is watching" is false in code
      (`guardrail_events`, push-send, the catalog, update checks, repo
      OAuth). Twelve founder decisions are listed in order, five before
      Phase 0. Nothing built.
- [ ] **Repositories: reconnect GitHub, then a device pass (2026-09-25).**
      ...
      and confirm a new chat works in it. Before others connect: make the
      GitHub App public; add `VITE_GITHUB_CLIENT_ID` as a repository variable.
...
### 2026-09-25, the v1 direction reviewed by the advisor team: go with conditions

Founder: advice set the v1 direction ("Your models, your keys, your machine, everywhere you go, and no one, including us, is watching"; three phases; a 90-second recording as done); have the team review it for a consensus go or no-go that closes the gap, and build nothing. All eight advisors returned GO WITH CONDITIONS, recorded in `docs/v1-direction-review.md` and `docs/v1-direction-advisory-memos.md`. The gaps, verified in code: a phone session never runs verify (`loop.ts:341`, `remote-attached`), undo has no daemon route, and verify's `execSync` runs in the Electron main process; the only measured floor model is non-commercial; the promise line is false today (signed-in blocks and likeness records post to our `guardrail_events` with the named person, push goes through our push-send, the catalog is fetched daily, update checks run every 30 minutes with auto-download, the repo OAuth exchange runs on our edge function, local models search DuckDuckGo by default); `*.ts.net` counts as local, Funnel hosts included; unsigned Mac and Windows builds block strangers; and 90 unedited seconds cannot hold setup. The review amends each phase, adds a stranger launch gate, gives the hide list with votes, and lists twelve founder decisions in order.
...
### 2026-09-25, setup is a choice after Harbor: start chatting or keep setting up

Founder: once Harbor Lite has offered Harbor, ask whether to start a chat or keep setting up, instead of rolling into the next step, so anyone can step past setup. Built: whether Harbor was taken (downloading or ready) or skipped, the guide says so and asks "Want to start chatting now, or keep setting up?", naming the steps still ahead, with two buttons (Start chatting, Keep setting up) and Harbor's download progress still showing. The walk holds until a tap (`nextChoice: 'asking'`, `chooseNext`), the same way the edit choice does. Start chatting pauses the walk where it stands: a "Let's chat" line with a few openers under it, Pick up setup under the hello, and Harbor's ready line still lands. Keep setting up goes straight into the next step. Typed answers work too: "keep setting up", "let's set up", or "next" carry on, a bare "skip" chats, and "not now" chats and still goes to the guide. No fork when nothing else is left to set up. Harbor Lite's setup card and the model's walk line know about the fork. Tests: `app/test/guidedSetup.test.ts`. Not yet felt on a phone.

### 2026-09-25, every local model searches the web; an image asks before it leaves

Founder, after a Stack chat with Harbor could not look anything up: every local model searches the web, on the provider Settings picks (DuckDuckGo with no key, or the person's own Brave, Tavily, or Perplexity). Before this, only Harbor in its own chat searched; in a Stack chat every local model was told it had no web access. Now a pocket model (Harbor, Harbor Lite, any downloaded model), your own model server (BYOM, text turns), and the desktop's free chat all search. Harbor Lite's guide harness searches for it before it writes; the others ask with one `SEARCH:` line. The rule lives once in `os-code/src/harness/localSearch.ts` (`SEARCH_PROTOCOL_NOTE`, `parseSearchLine`, and `SearchLineFilter`, which keeps the line off the screen while it streams), and the phone side is `app/src/lib/localSearch.ts`. There is one search per message, and on the Offline profile the model is told plainly it has no web. The desktop's `/chat` only adds the instruction when asked (`search: true`); the phone runs the search, so the desktop still fetches nothing for that surface. Images, founder's call: "Ask first unless the stack has a model there that is dedicated to image reading, then it's just automatic." Search cannot read a photo (it needs words, and DuckDuckGo has no image search), and no local runtime here reads images. So a model placed for Image reading gets the image with no card; any other reader (the anchor, or a connected cloud model filling the gap) waits on a card: amber Cloud spend for a cloud reader ("Read this image with Claude?"), a plain one for your own server. Nothing is sent before Approve, a No ends the turn and points to Image reading in Stack, and a stop counts as a No. Tests: `os-code/test/localSearch.test.ts`, `app/test/localModelSearch.test.ts`, `app/test/stackImageAskFirst.test.ts`. Not yet felt on a phone.
