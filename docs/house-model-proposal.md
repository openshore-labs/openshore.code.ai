...
| Bonsai 2 27B (PrismML, ternary)      | A 27B on a Qwen3.8 base compressed to ternary weights (about 1.76 bits each), roughly 6 GB total, Apache-2.0, long context. If the quality holds it collapses the fit table: a 27B-class seat that fits from 8 GB up, not only on a hub. Founder flagged it 2026-09-21. | EVAL FIRST, and be skeptical. The 83.9 aggregate and "98.2% retention" are the vendor's and blog numbers, not an agentic-coding run; ternary compression is exactly where precise multi-line edits, two-file refactors, and exact SEARCH/REPLACE slip. An independent tester already questioned whether v1's lead held at suite level and whether v2's agentic-coding claim holds. Confirm the Ollama/GGUF source and the license on the card, then measure it on the deep eval against the 14B and 32B, weighting the edit and refactor tasks, before it is offered as a size. On a literal 8 GB machine the ~6 GB is weights only, so context and the OS make it tight; comfortable from 16 GB. |
