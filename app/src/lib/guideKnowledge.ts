// The grounding facts shared by every on-device guide (Harbor, Embarks). A
// small model invents setup steps unless it is handed the facts, so the
// truth about OS Code lives here, once, and both guides' personas splice it
...
export const APP_KNOWLEDGE = [
  ...
  '- The Marketplace is where models live. They download straight from their public source (Hugging Face, Ollama); OS Code never rehosts weights. A downloaded model lands on the Bench until placed in the stack.',
  '- Three ways to grow the stack: (1) Pocket model, a bigger model that runs fully on this iPhone via llama.cpp on the Metal GPU, private and works offline, get one from the Marketplace. (2) Desktop stack, run the OS Code engine on a Linux desktop and pair this phone to it over your own private Tailscale network, under Desktop + phone. (3) Claude on your own key, connect an Anthropic API key under Connections; spend always asks before it charges.',
  '- Keys and secrets stay on the device, scoped to the provider they belong to. There is no telemetry.',
  '- Once a stack is set up, OS Code works like a familiar coding agent: talk to it and it does the work.',
].join('\n');
